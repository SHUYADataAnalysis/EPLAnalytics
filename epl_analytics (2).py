...
[data-testid="stAppViewContainer"] > .main > .block-container {{
  ...
  color: {C['chalk']} !important;
  font-family: 'Inter', sans-serif;
}}

/* すべてのStreamlitテキスト要素を黒に統一 */
...
h1 {{
  ...
  letter-spacing:.05em; color:#ffffff; margin:0; line-height:1.1;
}}
h2 {{
  font-family:'Bebas Neue',sans-serif; font-size:1.5rem;
  letter-spacing:.04em; color:{C['pitch']}; margin:.5rem 0 .3rem;
}}
...
.pill-green {{ background:{C['pitch_l']}; color:#ffffff; }}
.pill-amber {{ background:{C['amber']};   color:#ffffff; }}
.pill-sky   {{ background:{C['sky']};     color:#ffffff; }}

/* セクションバー */
.section-bar {{
  height:4px;
  background:linear-gradient(90deg,{C['pitch']},{C['amber']},{C['sky']},transparent);
  margin:.5rem 0 1rem; border-radius:2px;
}}

/* フッター */
.footer {{
  background:{C['dark']}; color:#94a3b8; font-size:.7rem;
...
[data-testid="stDataFrame"] * {{ color: {C['chalk']} !important; }}

/* タブ */
.stTabs [data-baseweb="tab-list"] {{
  gap:4px; border-bottom:2px solid {C['pitch']};
  background:transparent;
}}
.stTabs [data-baseweb="tab"] {{
  background:#e8f0eb; border-radius:6px 6px 0 0;
  color:{C['chalk']}; font-weight:600; font-size:.82rem;
  padding:6px 14px;
}}
.stTabs [aria-selected="true"] {{
  background:{C['pitch']}; color:#ffffff !important;
}}
...
def fetch_finished_fixture_ids(season_str: str, api_key: str) -> list:
    ...
    if not data or "response" not in data:
        return []
    return [f["fixture"]["id"] for f in data["response"]]
...
def fetch_and_cache_stats(
    ...
    """
    未取得の試合だけAPIを叩いてセッションキャッシュに保存。
    max_per_run: 1回の実行での最大取得数（100req/日制限に収まるよう調整）
    """
    import json as _json, time as _time

    cache_key = "apf_fixture_cache"
    ...
    if to_fetch:
        ...
        for i, fid in enumerate(to_fetch):
            msg.caption(f"📡 取得中: {i+1}/{len(to_fetch)} 試合 (残リクエスト: ~{max_per_run - i})")
            ...
            _time.sleep(0.35)    # per-minute レート制限を守る
        prog.empty(); msg.empty()

    st.session_state[cache_key] = _json.dumps(cached)
...
st.sidebar.markdown("---")
st.sidebar.markdown("**⚡ API-Football（シュート・ポゼッション等）**")
api_key_input = st.sidebar.text_input(
    "API Key（無料登録: api-sports.io）",
    value=st.secrets.get("APIFOOTBALL_KEY", "") if hasattr(st, "secrets") else "",
    type="password",
    help="取得方法: https://dashboard.api-football.com/register"
)
apf_enabled = bool(api_key_input.strip())
if apf_enabled:
    apf_valid, apf_used, apf_remain = check_apf_key(api_key_input.strip())
    if apf_valid:
        st.sidebar.success(f"✅ 接続OK  本日残り: {apf_remain}req")
    else:
        st.sidebar.error("❌ APIキーが無効です")
        apf_enabled = False
else:
    st.sidebar.caption("⚡マークの指標はAPIキーが必要です")
st.sidebar.markdown("---")
...
if apf_enabled:
    with st.spinner("⚡ API-Footballから試合スタッツを取得中..."):
        fixture_ids = fetch_finished_fixture_ids(season, api_key_input.strip())
        n_total     = len(fixture_ids)
        cache_raw   = st.session_state.get("apf_fixture_cache", "{}")
        import json as _j
        n_cached = len([k for k in _j.loads(cache_raw if cache_raw else "{}") 
                        if int(k) in fixture_ids])
        st.info(
            f"⚡ 試合スタッツ: {n_cached}/{n_total} 試合取得済み  "
            f"(本日残り: {st.session_state.get('apf_remain_today', apf_remain)}req)"
        )
        # 未取得分を取得（1日100req制限に合わせて最大80試合/回）
        if n_cached < n_total and apf_remain > 5:
            fixture_cache = fetch_and_cache_stats(
                fixture_ids, api_key_input.strip(),
                max_per_run=min(apf_remain - 5, 80)
            )
        else:
            try:
                fixture_cache = {int(k): v for k, v in _j.loads(cache_raw).items()
                                 if int(k) in fixture_ids}
            except Exception:
                fixture_cache = {}
...
<div style="background:{C['dark']};border-radius:10px;padding:1rem 1.5rem;
  ...
  <h1>EPL Analytics</h1>
  <div style="color:#94a3b8;font-size:.82rem;font-weight:600;letter-spacing:.04em;margin-top:.2rem">
